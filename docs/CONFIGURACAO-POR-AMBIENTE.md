...
|-------|------|-------------|
| **URL do backend** | `js/config.js` → `COMPRAS_COLETIVAS_API` | O repo traz um placeholder; substitua pela **sua** URL do Web App (Apps Script) ou do proxy CORS. Sem isso o site carrega mas as chamadas da Compra Coletiva falham. No seu fork (ex.: GitHub Pages), edite e faça push para o site usar sua API. |
